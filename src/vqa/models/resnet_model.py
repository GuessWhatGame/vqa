...
class ResnetModel(AbstractNetwork):
    ...
    def get_parameters(self, fine_tuned=[]):

        params = [v for v in tf.trainable_variables() if
                (not 'resnet' in v.name or 'cbn_input' in v.name)]
    ...
    def get_resnet_parameters(self):
        ...
        return [v for v in tf.global_variables() if
            ('resnet' in v.name and
